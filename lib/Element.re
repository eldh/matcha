...
/* Where an [Overlay] sits in the FRAME (B1). Horizontal placement is always
 * centred - a floating layer that is not centred horizontally reads as a
 * mistake in a terminal - so only the vertical axis is a choice:
 * - OverlayCenter: vertically centred in the frame. The obvious default, and
 *   the wrong one for an INLINE app: centring a layer in a 40-row terminal
 *   grows the live region to 40 rows even if the app itself is 6 rows tall.
 * - OverlayTop(n): n rows down from the top of the frame.
 * - OverlayBottom(n): n rows up from the bottom of the frame. */
type overlayAlign =
  | OverlayCenter
  | OverlayTop(int)
  | OverlayBottom(int);

/* Options of an [Overlay] node (see [t] below) - the float primitive
 * <Modal> builds on.
 *
 * - ovWidth / ovHeight: the box, resolved against the FRAME (never against
 *   the enclosing slot - a modal opened from deep inside a pane is still a
 *   frame-level object). Chars(n) is clamped to the frame, Percent(p) is p%
 *   of it, Flex(_) means the whole frame, and Auto is the child's natural
 *   size clamped to the frame.
 * - ovAlign: see [overlayAlign].
 * - ovShadow: paint a drop shadow to the lower-right. The shadow ADDS [Dim]
 *   to the cells underneath rather than painting over them, so whatever the
 *   application is drawing there stays readable through it.
 * - ovOnDismiss: run when a mouse Down lands OUTSIDE the box. That click is
 *   then swallowed whole - it must not also press whatever it landed on. */
type overlayOptions = {
  ovWidth: size,
  ovHeight: size,
  ovAlign: overlayAlign,
  ovShadow: bool,
  ovOnDismiss: option(unit => unit),
};
...
type t =
  ...
  | Overlay(t, overlayOptions)
  /* Overlay(child, options): a FLOATING LAYER (B1) - content painted over
   * the finished frame, owning input while it is open.
   *
   * INVISIBLE TO LAYOUT, exactly like <Static>: it renders to "" wherever it
   * sits and Runtime's stacks filter it out of every layout computation
   * before they run, so putting one in a VStack costs no row, no gap slot
   * and no justify share. What it actually does happens as a side effect of
   * the render: the child is rendered into its own box, in FRAME
   * coordinates, and the result is queued for the frame's composite pass
   * (Runtime.compositeOverlays), which splices it over the base frame after
   * layout has finished.
   *
   * IT IS ALSO A CONTAINER. The box is pushed onto Runtime.containerStack
   * while the child renders, so [Matcha.useContainerSize] inside a dialog
   * reports the DIALOG, not the window - responsive dialog content with no
   * extra API. It pushes a clip rect too (box intersected with the frame,
   * NOT with the enclosing clip: a modal opened from inside a <ScrollView>
   * must not inherit that scroller's window).
   *
   * MEMBERSHIP - who the layer owns - is exactly what is rendered INSIDE
   * [child]. That is what [Hooks.useInput] is filtered against while a layer
   * is open, what focus is contained to, and what the mouse can hit. A
   * component that merely RETURNS an Overlay is therefore not a member of
   * its own layer, and would be suppressed by it; that, plus the zero-layout
   * rule above, is why <Modal> is Lazy(() => Overlay(component)) and not a
   * component wrapping an Overlay. See lib/Modal.re.
   *
   * [Element.render] returns "": detached rendering has no frame to float
   * over, and no composite pass to hand the layer to. */
...
/* Float an element over the finished frame. Occupies no layout space; see
 * the Overlay variant, and prefer <Modal> for the usual dialog case. */
let overlay = (~options: overlayOptions, el: t): t => Overlay(el, options);
...
let rec render = (el: t): string => {
  ...
  | Container(child) =>
    ...
    render(child)
  | Overlay(_child, _options) =>
    /* Nothing, for the same reason as Static below: an overlay's output does
     * not belong to the string its parent is building. It is composited over
     * the FINISHED FRAME by Runtime.compositeOverlays, and a detached render
     * has no frame to float over, no frame size to resolve the box against,
     * and no composite pass to hand the layer to. Rendering the child here
     * would splice dialog content inline, in the middle of whatever the
...
/* Overlay component - the raw floating layer. Most applications want
 * <Modal> (lib/Modal.re) instead: it adds the border, the Esc binding and
 * the focus save/restore, and it takes an ~isOpen prop, whereas an <Overlay>
 * in the tree is ALWAYS open (wrap it in the usual `cond ? ... : Empty`
 * conditional to toggle it).
 *
 * Usage:
 *   <Overlay width={Percent(50)} height={Chars(8)} align={OverlayTop(2)}>
 *     <MyDialog />
 *   </Overlay>
 *
 * Put it directly in a stack - it occupies no layout space there (see the
 * [Overlay] variant). Everything rendered inside it is a MEMBER of the
 * layer: its useInput fires while the layer is topmost, its useFocus is what
 * Tab is contained to, and it is what the mouse can hit.
 */
module Overlay = {
  let make =
      (
        ...
        ~height: size=Auto,
        ~align: overlayAlign=OverlayCenter,
        ~shadow: bool=true,
  ...
  let createElement =
    ...
    Lazy(() =>
      make(~children, ~width, ~height, ~align, ~shadow, ~onDismiss?, ())
    );
};
